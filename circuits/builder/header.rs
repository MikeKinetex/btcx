...
impl<L: PlonkParameters<D>, const D: usize> BitcoinHeaderVerify<L, D> for CircuitBuilder<L, D> {
    ...
    fn validate_header(
        ...
        // calculate work
        let work = self.calculate_work(&threshold);
...
mod test {
    ...
    fn test_validate_header() {
        ...
        builder.write(header.hash);
        builder.write(header.work);
        ...
        let hash = _output.read::<BlockHashVariable>();
        let work = _output.read::<WorkVariable>();

        let (exp, mantissa) = compute_exp_and_mantissa(header_input);
        let threshold = compute_threshold(exp, mantissa);
        let expected_work = U256::from_little_endian(
            compute_work(threshold).to_bytes_le().as_slice()
        );

        log::debug!("Hash: {:?} = {:?}", hash, expected_hash);
        log::debug!("Work: {:?} = {:?}", work, expected_work);
